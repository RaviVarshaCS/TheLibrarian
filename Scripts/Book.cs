...
public class Book : MonoBehaviour
{
    ...
    // List to store books
    public List<Book_> books; // List of all available books

    private Dictionary<string, Book_> bookDictionary;

    // A UI element to display the book (this could be a Canvas, 3D object, etc.)
    public GameObject bookDisplayArea;
    ...
    private void Awake()
    {
        ...
        bookDictionary = new Dictionary<string, Book_>();

        // Populate the dictionary with books
        LoadBookAssets();
    }

    private void LoadBookAssets()
    {
        // Convert the list to a dictionary for faster lookup by book name
        foreach (Book_ book in books)
        {
            if (!bookDictionary.ContainsKey(book.name))
            {
                bookDictionary.Add(book.name, book);
            }
            else
            {
                Debug.LogWarning("Book with the name " + book.name + " already exists.");
            }
        }
    }

    // Method to load the correct book's assets based on the book name
    public void LoadBook(string bookName)
    {
        if (bookDictionary.ContainsKey(bookName))
        {
            Book_ selectedBook = bookDictionary[bookName];

            // Load the book's asset and pages
            Debug.Log("Loading asset for: " + selectedBook.name);
            ShowBookPages(selectedBook);
        }
        else
        {
            Debug.LogError("No asset found for book: " + bookName);
        }
    }

    // Display the pages or assets associated with the book
    private void ShowBookPages(Book_ selectedBook)
    {
        // Example logic to show the book pages or assets
        // You can display the book asset (3D model) and pages (e.g., in UI or 3D space)
        
        // Example: Display the book asset in the display area
        if (bookDisplayArea != null && selectedBook.bookAsset != null)
        {
            // Assuming the book display area is a placeholder for displaying the book in the UI or 3D world
            GameObject bookInstance = Instantiate(selectedBook.bookAsset, bookDisplayArea.transform.position, Quaternion.identity);
            bookInstance.transform.SetParent(bookDisplayArea.transform); // Set parent to manage UI space or 3D space
            bookInstance.SetActive(true);
        }

        // Now display the pages of the book (For example, show them in a UI or 3D space)
        Debug.Log("Showing pages for: " + selectedBook.name);
        foreach (Sprite page in selectedBook.pages)
        {
            Debug.Log("Page: " + page.name); // This can be replaced with your actual display logic
            // Example: Load the pages into a UI element (e.g., Image component in Unity)
            if (bookDisplayArea != null)
            {
                // You could assign the page to an Image component or a similar UI element
                // Example:
                Image imageComponent = bookDisplayArea.GetComponent<Image>();
                if (imageComponent != null)
                {
                    imageComponent.sprite = page;
                }
            }
        }
    }
