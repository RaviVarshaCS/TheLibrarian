...
//     // public GameObject bookDisplayArea;
...
//     public void SetBookPages(Sprite[] pages)
//     {
//         bookPages = pages;

//         Debug.Log("UI Book pages set");
//     }
...
public class Book : MonoBehaviour {
    ...
    public bool interactable = true;
    public bool enableShadowEffect = true;
    // represents the index of the sprite shown in the right page
    public int currentPage = 0;
    public int TotalPageCount
    {
        get { return bookPages.Length; }
    ...
    private void CalcCurlCriticalPoints()
    {
        ...
        float pageHeight = BookPanel.rect.height;
        radius2 = Mathf.Sqrt(pageWidth * pageWidth + pageHeight * pageHeight);
        Debug.Log("CalcCurlCriticalPoints: sb=" + sb + ", st=" + st + ", ebr=" + ebr + ", ebl=" + ebl + ", radius1=" + radius1 + ", radius2=" + radius2);
    }

    public Vector3 transformPoint(Vector3 mouseScreenPos)
    {
        // Note: Uncomment the next line if you want to see every call (may be verbose).
        // Debug.Log("transformPoint called with mouseScreenPos: " + mouseScreenPos);
        if (canvas.renderMode == RenderMode.ScreenSpaceCamera)
        {
            Vector3 mouseWorldPos = canvas.worldCamera.ScreenToWorldPoint(new Vector3(mouseScreenPos.x, mouseScreenPos.y, canvas.planeDistance));
            Vector2 localPos = BookPanel.InverseTransformPoint(mouseWorldPos);
    ...
    {
        f = Vector3.Lerp(f, transformPoint(Input.mousePosition), Time.deltaTime * 10);
        Debug.Log("UpdateBook: f updated to: " + f);
        if (mode == FlipMode.RightToLeft)
            UpdateBookRTLToPoint(f);
        else
            UpdateBookLTRToPoint(f);
    ...
    public void UpdateBookLTRToPoint(Vector3 followLocation)
    {
        ...
        ShadowLTR.rectTransform.SetParent(Left.rectTransform, true);
        Debug.Log("UpdateBookLTRToPoint: followLocation=" + followLocation + ", c=" + c + ", t1=" + t1 + ", clipAngle=" + clipAngle);
    }
    ...
    public void UpdateBookRTLToPoint(Vector3 followLocation)
    {
        ...
        Shadow.rectTransform.SetParent(Right.rectTransform, true);
        Debug.Log("UpdateBookRTLToPoint: followLocation=" + followLocation + ", c=" + c + ", t1=" + t1 + ", clipAngle=" + clipAngle);
    }
    ...
    private float CalcClipAngle(Vector3 c, Vector3 bookCorner, out Vector3 t1)
    {
        ...
        float T0_CORNER_dy = bookCorner.y - t0.y;
        float T0_CORNER_dx = bookCorner.x - t0.x;
        float T0_CORNER_Angle = Mathf.Atan2(T0_CORNER_dy, t0.x - bookCorner.x); // adjusted for clarity
        float T0_T1_Angle = 90 - T0_CORNER_Angle;
        ...
        float T0_T1_dx = t1.x - t0.x;
        T0_T1_Angle = Mathf.Atan2(T0_T1_dy, T0_T1_dx) * Mathf.Rad2Deg;
        Debug.Log("CalcClipAngle: c=" + c + ", bookCorner=" + bookCorner + ", t0=" + t0 + ", t1=" + t1 + ", T0_T1_Angle=" + T0_T1_Angle);
        return T0_T1_Angle;
    }
    ...
    private Vector3 Calc_C_Position(Vector3 followLocation)
    {
        ...
        if (C_ST_distance > radius2)
            c = r2;
        Debug.Log("Calc_C_Position: followLocation=" + followLocation + ", c=" + c);
        return c;
    }

    public void DragRightPageToPoint(Vector3 point)
    {
        Debug.Log("DragRightPageToPoint called with point: " + point);
        if (currentPage >= bookPages.Length)
        {
            Debug.Log("DragRightPageToPoint: currentPage (" + currentPage + ") >= bookPages.Length (" + bookPages.Length + ")");
            return;
        }
        pageDragging = true;
        mode = FlipMode.RightToLeft;
        f = point;
        Debug.Log("DragRightPageToPoint: pageDragging set to true, mode: RightToLeft, starting point f: " + f);
    ...
    public void OnMouseDragRightPage()
    {
        Debug.Log("OnMouseDragRightPage triggered.");
        if (interactable)
            DragRightPageToPoint(transformPoint(Input.mousePosition));
    }
    ...
    public void DragLeftPageToPoint(Vector3 point)
    {
        Debug.Log("DragLeftPageToPoint called with point: " + point);
        if (currentPage <= 0)
        {
            Debug.Log("DragLeftPageToPoint: currentPage (" + currentPage + ") <= 0");
            return;
        }
        pageDragging = true;
        mode = FlipMode.LeftToRight;
        f = point;
        Debug.Log("DragLeftPageToPoint: pageDragging set to true, mode: LeftToRight, starting point f: " + f);
    ...
    public void OnMouseDragLeftPage()
    {
        Debug.Log("OnMouseDragLeftPage triggered.");
        if (interactable)
            DragLeftPageToPoint(transformPoint(Input.mousePosition));
    }
    ...
    public void OnMouseRelease()
    {
        Debug.Log("OnMouseRelease triggered.");
        if (interactable)
            ReleasePage();
    }
    ...
    public void ReleasePage()
    {
        Debug.Log("ReleasePage called.");
        if (pageDragging)
        {
            pageDragging = false;
            float distanceToLeft = Vector2.Distance(c, ebl);
            float distanceToRight = Vector2.Distance(c, ebr);
            Debug.Log("ReleasePage: distanceToLeft=" + distanceToLeft + ", distanceToRight=" + distanceToRight + ", mode=" + mode);
            if (distanceToRight < distanceToLeft && mode == FlipMode.RightToLeft)
                TweenBack();
            else if (distanceToRight > distanceToLeft && mode == FlipMode.LeftToRight)
                TweenBack();
    ...
    void UpdateSprites()
    {
        LeftNext.sprite = (currentPage > 0 && currentPage <= bookPages.Length) ? bookPages[currentPage - 1] : background;
        RightNext.sprite = (currentPage >= 0 && currentPage < bookPages.Length) ? bookPages[currentPage] : background;
        Debug.Log("UpdateSprites: currentPage=" + currentPage);
    }

    public void TweenForward()
    {
        Debug.Log("TweenForward called, mode: " + mode);
        if (mode == FlipMode.RightToLeft)
            currentCoroutine = StartCoroutine(TweenTo(ebl, 0.15f, () => { Flip(); }));
        else
            currentCoroutine = StartCoroutine(TweenTo(ebr, 0.15f, () => { Flip(); }));
    ...
    void Flip()
    {
        Debug.Log("Flip method called. mode: " + mode + ", currentPage before: " + currentPage);
        if (mode == FlipMode.RightToLeft)
            currentPage += 2;
        else
            currentPage -= 2;
        Debug.Log("Flip method: new currentPage: " + currentPage);
        LeftNext.transform.SetParent(BookPanel.transform, true);
        Left.transform.SetParent(BookPanel.transform, true);
        LeftNext.transform.SetParent(BookPanel.transform, true);
        Left.gameObject.SetActive(false);
    ...
    public void TweenBack()
    {
        Debug.Log("TweenBack called, mode: " + mode);
        if (mode == FlipMode.RightToLeft)
        {
            currentCoroutine = StartCoroutine(TweenTo(ebr, 0.15f,
                () =>
                {
                    Debug.Log("TweenBack callback for RightToLeft executed.");
                    UpdateSprites();
                    RightNext.transform.SetParent(BookPanel.transform);
                    Right.transform.SetParent(BookPanel.transform);
        ...
        else
        {
            currentCoroutine = StartCoroutine(TweenTo(ebl, 0.15f,
                () =>
                {
                    Debug.Log("TweenBack callback for LeftToRight executed.");
                    UpdateSprites();
    ...
    public IEnumerator TweenTo(Vector3 to, float duration, System.Action onFinish)
    {
        Debug.Log("TweenTo called with target: " + to + " and duration: " + duration);
        int steps = (int)(duration / 0.025f);
        Vector3 displacement = (to - f) / steps;
        for (int i = 0; i < steps - 1; i++)
        {
